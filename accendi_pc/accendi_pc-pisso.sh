...
# Rilevazione OS e package manager
OS_TYPE="unknown"
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    OS_TYPE="Windows"
    PACKAGE_MANAGER="choco" # oppure "winget" se preferisci
...
then
    echo ""
    echo -e "${RED}Il comando 'wakeonlan' non è installato.${RESET}"
    ...
    if [[ "$risposta" =~ ^[Yy]$ ]]; then
        case "$PACKAGE_MANAGER" in
            ...
            brew)
                ...
                ;;
            choco)
                echo -e "${CYAN}Installo wakeonlan con chocolatey...${RESET}"
                choco install wakeonlan -y
                ;;
            *)
